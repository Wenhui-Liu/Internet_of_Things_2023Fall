...
public class Collect_PT extends AppCompatActivity implements SensorEventListener {
    ...
    private Classifier cls_MP = null;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_collect_pt);
        ...
        collect_button_c.setOnClickListener(new View.OnClickListener() {
            ...
            {
                Toast.makeText(Collect_PT.this, "Currently, The Collect Interface", Toast.LENGTH_SHORT).show();
            }
        } );
        train_button_c.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v)
            {
                ...
                double[] results_MP = classification(cls_MP, min, max, std, zcr, mean, var);
                int predictedClass_MP = (int) results_MP[0];
                double confidence_MP = results_MP[1];

                Intent Train_intent = new Intent(Collect_PT.this, Train_PT.class);
                Train_intent.putExtra("PT_DT_confidence", confidence_DT);
                Train_intent.putExtra("PT_DT_predicted_class", predictedClass_DT);
                Train_intent.putExtra("PT_RF_confidence", confidence_RF);
                Train_intent.putExtra("PT_RF_predicted_class", predictedClass_RF);
                Train_intent.putExtra("PT_NB_confidence", confidence_NB);
                Train_intent.putExtra("PT_NB_predicted_class", predictedClass_NB);
                Train_intent.putExtra("PT_MP_confidence", confidence_MP);
                Train_intent.putExtra("PT_MP_predicted_class", predictedClass_MP);
        ...
        deploy_button_c.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v)
            {
                Intent intent = new Intent(Collect_PT.this, Deploy_PT.class);
        ...
        sensorManager.registerListener(Collect_PT.this,accelerometer,sensorManager.SENSOR_DELAY_NORMAL);

        start_button_c.setBackgroundColor(Color.parseColor("#ADD8E6"));
        end_button_c.setBackgroundColor(Color.parseColor("#ADD8E6"));
        try {
            cls_DT = (Classifier) read(getAssets().open("DecisionTree_PT.model"));
            cls_RF = (Classifier) read(getAssets().open("RandomForest_PT.model"));
            cls_NB = (Classifier) read(getAssets().open("NaiveBayes_PT.model"));
            cls_MP = (Classifier) read(getAssets().open("MultilayerPerceptron_PT_2.model"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        end_button_c.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isCollecting) {
                    sensorManager.unregisterListener(Collect_PT.this);
    ...
    public double [] classification(Classifier cls, double Min, double Max, double StdDev, double ZeroCrossingRate, double Mean, double Variance) {
        ...
        try {
            ...
            e.printStackTrace();
            Toast.makeText(Collect_PT.this, "Error: " + String.valueOf(e) , Toast.LENGTH_LONG).show();
        }catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
